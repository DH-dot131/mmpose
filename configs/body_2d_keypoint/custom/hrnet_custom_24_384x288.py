...
val_evaluator = dict(type='NME',norm_mode = 'keypoint_distance', keypoint_indices = [0,21])
test_evaluator = dict(type='NME',norm_mode = 'keypoint_distance', keypoint_indices = [0,21])
...
# 2) early stopping을 custom_hooks 로 옮기고 patience와 priority 조정
custom_hooks = [
    dict(type='SyncBuffersHook'),
    dict(
        type='EarlyStoppingHook',
        monitor='NME',  # 실제 로그에 찍히는 metric 키로 바꿔주세요
        rule='less',
        patience=5,               # 6회 검증(=30 epoch)까지 대기
        priority='LOW'            # checkpoint 이후에 실행되도록 낮은 우선순위
    ),
]
